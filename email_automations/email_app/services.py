...
class EmailSender:
    ...
    def send_simple_email(self, 
        ...
        try:
            ...
            # Use exact email address in From field - some SMTP servers require exact match with authenticated email
            # This is critical for SpaceMail and other strict SMTP servers
            message['From'] = self.email
            message['To'] = to_email
            message['Subject'] = subject
            message['Date'] = email.utils.formatdate(localtime=True)
            
            # Use spacemail.com domain for Message-ID to ensure consistency
            # Some mail servers are strict about Message-ID domain matching the sending server
            message['Message-ID'] = make_msgid(domain='spacemail.com')
            message['Reply-To'] = self.email
            
            # Add additional headers for better deliverability
            # Note: Return-Path is typically set by the receiving server, but we set it for compatibility
            # For SpaceMail, we use the sender's email to maintain SPF alignment
            message['Return-Path'] = self.email
            # MIME-Version is required for proper email formatting
            message['MIME-Version'] = '1.0'
            # X-Mailer header to identify the sending application
            message['X-Mailer'] = 'SpaceMail Automation System'
            # X-Originating-IP can help with deliverability (optional)
            # Add sender's domain info for better tracking
            sender_domain = self.email.split('@')[1] if '@' in self.email else 'unknown'
            message['X-Sender-Domain'] = sender_domain
            ...
            with self._get_smtp_connection(timeout=30) as server:
                # Enable debug output to see SMTP conversation (helpful for troubleshooting)
                ...
                print(f"   📧 From address: {self.email}")
                print(f"   📧 To addresses: {recipients}")
                
                failed_recipients = server.sendmail(self.email, recipients, message.as_string())
                
                if failed_recipients:
                    print(f"   ⚠ Warning: Some recipients failed: {failed_recipients}")
                    return False
                else:
                    print(f"   ✅ SMTP server accepted email for all recipients")
                
                # Disable debug after sending
                server.set_debuglevel(0)
            
            print(f"✓ Email sent successfully to {to_email} (From: {self.email})")
            return True
    ...
    def send_html_email(self,
        ...
        try:
            ...
            print(f"✓ HTML email sent successfully to {to_email} (From: {self.email})")
            return True
    ...
    def send_email_with_attachment(self,
        ...
        try:
            ...
            print(f"✓ Email with attachment sent successfully to {to_email} (From: {self.email})")
            return True
