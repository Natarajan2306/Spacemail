...
def send_email_api(request):
    ...
    try:
        ...
        if gmail_username and gmail_password:
            ...
            # Use SpaceMail: check session first, then SystemProviderSettings, then defaults
            from email_app.models import SystemProviderSettings
            
            smtp_username = request.session.get('smtp_username')
            smtp_password = request.session.get('smtp_password')
            
            # If no session credentials, try SystemProviderSettings
            if not smtp_username or not smtp_password:
                system_provider = SystemProviderSettings.get_active_provider()
                if system_provider and system_provider.provider_type == 'spacemail':
                    smtp_username = system_provider.smtp_username
                    smtp_password = system_provider.smtp_password
            
            # Final fallback to settings (for backward compatibility)
            smtp_username = smtp_username or settings.SENDER_EMAIL
            smtp_password = smtp_password or settings.SENDER_PASSWORD
            smtp_server = 'mail.spacemail.com'
            smtp_port = 465
        
        # Debug: Log which credentials are being used
        print(f"🔐 Using SMTP credentials: {smtp_username} on {smtp_server}:{smtp_port}")
        print(f"   Session has credentials: {bool(request.session.get('smtp_username'))}")
